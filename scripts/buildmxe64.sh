...
CROSSTARGET=x86_64-w64-mingw32.static	# 64-bit
...
export CFLAGS=" -march=x86-64 -mtune=k8 -O2 -ffast-math -mno-3dnow -mmmx -msse -msse2 -mfpmath=sse "	# Optimizations compatible with all X86_64 CPUS
if $HARDEN; then
	CFLAGS+=$HARD_CFLAGS
...
CONFIGUREFLAGS=" --enable-optimizations=sse2 " 	# 64-bit
...
function Env_Variables
{
	...
	[ -z "$CROSSCFG" ] && export CROSSCFG="--build=$CROSSSTARGET --host=$CROSSTARGET"
