...
{ public:
  ...
  public:
   ...
   void Process(Type Input)
     { if(Len==0) return;
       Tap[TapPtr]=Input;
       TapPtr+=1; if(TapPtr>=Len) TapPtr-=Len; }
  ...
  public:
   ...
   void Free(void)
     { free(Energy);     Energy=0;
       free(Correction); Correction=0;
       free(Corrected);  Corrected=0; }
   ...
   void MakeRandom(Type SignalToNoise=1.0)
     { Type Signal=1.0;
       ...
       size_t Idx;
       for(Idx=0; Idx<Carriers; Idx++)
       { double UniformNoise=((double)rand()+1.0)/((double)RAND_MAX+1.0);
   ...
   int Scan_First(void)
     { for(ScanIndex=0; ScanIndex<Carriers; ScanIndex++)
       { if(Corrected[ScanIndex]>ScanThreshold) return 0; }
       return -1; } // 0 => OK, -1 => no data above threshold

   int Scan_Next(void)
     { for( ScanIndex++; ScanIndex<Carriers; ScanIndex++)
       { if(Corrected[ScanIndex]>ScanThreshold) return 0; }
       if(Scan_First()<0) return -1;
       return 1; } // 0 => OK, 1 => wrapped around, -1 => no data above threshold
...
// { 1.0, 1.0 } ;
 { +1.0000000000, +1.1913785723, -0.0793018558, -0.2171442026, -0.0014526076 } ;
...
{ public:
  ...
  public:
   ...
   void LimitSpectraPeaks(Cmpx<Type> *Spectra,
                          size_t Len=64, Type Threshold=4.0)
     { Filter.Len=Len;
       Filter.Preset();
...
{ public:
  ...
  public:
   ...
   int Preset(void)
     {
       ...
       Modulator.SymbolLen=(size_t)1<<(BitsPerSymbol+7-Log2(Bandwidth/125));
       Bandwidth=Exp2(Log2(Bandwidth/125))*125;
       Modulator.FirstCarrier=(size_t) ((Modulator.SymbolLen/16)*FirstCarrierMultiplier)+1;
   ...
   { return BitsPerSymbol*SampleRate/(SymbolsPerBlock*Modulator.SymbolSepar); }
...
{ public:
   ...
   Type   SyncThreshold;  // synchronizer S/N threshold (if below, output is suppressed)
   Type   SampleRate;     // audio sampling rate (internal processing)
  ...
  private:
   ...
   const static size_t SlicesPerSymbol = MFSK_Demodulator<Type>::SpectraPerSymbol;
   ...
   MFSK_SoftDecoder<Type,Type> RefDecoder; // reference decoder
   ...
   CircularBuffer<uint64_t> *DecodePipe; // pipeline for decoded FEC blocks
  ...
  public:
   ...
   int Preset(void)
     { size_t Idx;
       ...
       Bandwidth=Exp2(Log2(Bandwidth/125))*125;
       Demodulator.FirstCarrier=(size_t) ((Demodulator.SymbolLen/16)*FirstCarrierMultiplier)+1;
       ...
       if(ReallocArray(&Decoder,SlicesPerSymbol*FreqOffsets)<0) goto Error;
       for(Idx=0; Idx<(SlicesPerSymbol*FreqOffsets); Idx++)
       { Decoder[Idx].Init(); }
       for(Idx=0; Idx<(SlicesPerSymbol*FreqOffsets); Idx++)
   ...
   Type CharactersPerSecond(void)
   { return BitsPerSymbol*SampleRate/(SymbolsPerBlock*Demodulator.SymbolSepar); }

   Type SignalToNoiseRatio(void)
   { return SyncSNR; }

   Type FrequencyOffset(void)
   { return ((int)SyncBestFreqOffset-(int)FreqOffsets/2)*(SampleRate/Demodulator.SymbolLen); }

   Type TimeOffset(void)
   { return ((Type)SyncBestBlockPhase/SlicesPerSymbol)*(Demodulator.SymbolSepar/SampleRate); }
   ...
   void Flush(void)
     { ProcessInputBuffer();
	   ...
	   for(Idx=0; Idx<InputProcessor.WindowLen; Idx++)
	     InputBuffer[Idx]=0;
       size_t FlushLen=Demodulator.SymbolSepar*SymbolsPerBlock*SyncIntegLen*2;
  ...
  private:
    ...
    void ProcessInputBuffer(void)
     { while(InputBuffer.Len>=InputProcessor.WindowLen)
	   { InputProcessor.Process(InputBuffer.Elem);
         ...
         size_t Idx;
         for(Idx=0; Idx<InputProcessor.WindowLen; Idx+=Demodulator.SymbolSepar )
    ...
    void ProcessSymbol(InpType *Input)
   {
     ...
     { LowPass3_Filter<Type>
       ...
       { Demodulator.SoftDecode(Symbol,
                       Slice,(int)Offset-(FreqOffsets/2));
       ...
       { Type AverageSignal=0;
         size_t Idx;
         size_t Size=SyncSignal.Size;
         LowPass3_Filter<Type> *FilterPtr = SyncSignal.AbsPtr(0);
         for(Idx=0; Idx<Size; Idx++,FilterPtr++)
           AverageSignal+=FilterPtr->Output;
         AverageSignal-=SyncBestSignal;
         AverageSignal/=(Size-1);

         Type BestNoise=SyncNoiseEnergy.AbsPtr(SyncBestBlockPhase)
                                            [SyncBestFreqOffset].Output;
         if(BestNoise>0) BestNoise=sqrt(BestNoise);
		            else BestNoise=0;

         const Type MinNoise=0.001;
...
*/
         ...
         size_t Byte;
         for(Byte=0; Byte<BitsPerSymbol; Byte++)
