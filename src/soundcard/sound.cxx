...
SoundPulse::SoundPulse(const char *dev)
{
	...
	sd[0].buffer_attrs.fragsize = (uint32_t)-1;
	sd[0].buffer_attrs.tlength = SCBLOCKSIZE * sizeof(float);
...
long SoundPulse::src_read_cb(void* arg, float** data)
{
	...
	int err;
	int nread = 0;
	if ((nread = pa_simple_read(p->sd[0].stream, p->snd_buffer, 2 * sizeof(float) * p->sd[0].blocksize, &err)) == -1) {
...
size_t SoundPulse::Read(float *buf, size_t count)
{
	...
	if (progdefaults.RX_corr != 0) {
		...
		int err;
		if ((r = pa_simple_read(sd[0].stream, rbuf, 2 * sizeof(float) * count, &err)) == -1)
			throw SndPulseException(err);
	}

	// transfer active input channel; left == 0, right == 1
	for (size_t i = 0; i < count; i++) {
		n = sd[0].stream_params.channels * i;
		n += progdefaults.ReverseRxAudio;
		buf[i] = rbuf[n];
	}
