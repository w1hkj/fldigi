...
static void init_nibbles()
{
	...
	fname.append("nibbles.txt");
	fnibbles = fopen(fname.c_str(), "w");
	for (int i = 0; i < 199; i++) {
		orig = nibbles[i+165];
...
ifkp::ifkp(trx_mode md) : modem()
{
	...
	IMAGEspp = 16; // 16 samples per pixel
	pixfilter = new Cmovavg(IMAGEspp);
	ampfilter = new Cmovavg(IMAGEspp);
	syncfilter = new Cmovavg(3*IMAGEspp);//symlen / 32);
