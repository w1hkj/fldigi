...
//		}
		...
		if (_pskr && mode != MODE_PSK63F) Rxinlv2->symbols(twosym);
		...
		twosym[1] = symbolpair[1];
		// De-interleave
		if (_pskr && mode != MODE_PSK63F) Rxinlv->symbols(twosym);
		tempc = twosym[1];
		twosym[1] = twosym[0];
		twosym[0] = tempc;
		// Then viterbi decoder
	...
	if (_pskr) {
		...
		if (_16psk || _8psk || _xpsk) {
			if (!_disablefec) {
				...
				//printf("\n");
				if (_puncturing && _16psk) rx_pskr(128); // 16psk puncturer drops 2 bits/symbol
