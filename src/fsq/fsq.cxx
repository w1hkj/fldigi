...
static int nibbles[200];
static void init_nibbles()
{
	int nibble = 0;
	for (int i = -99; i < 99; i++) {
		nibble = floor(0.5 + i/3.0);
...
void  fsq::tx_init(SoundBase *sc)
{
	...
	double flo = 1000.0 / samplerate;
	double fhi = 2000.0 / samplerate;
	xmtfilt->init_bandpass (129, 1, flo, fhi);
}
...
// look for valid start protocol
	size_t p = rx_text.find(FSQBOL);
	if (p != NIT)
		rx_text.erase(0, p + 2);
	...
	size_t start_pos = 0;
	station_calling = rx_text.substr(start_pos, p);

	for (;;) {
		if (valid_callsign(station_calling) &&
			crc.sval(station_calling) == rxcrc) break;
		start_pos++;
		if (start_pos == p) break;
		station_calling = rx_text.substr(start_pos,p);
	}

	if (start_pos == p) { // valid calling station not identified
		rx_text.clear();
		return;
	}
	...
	std::string sheard = station_calling;
	sheard.append(",").append(inpTimeOff->value());
	sheard.append(",").append(szestimate).append("\n");
	heard_log << sheard;
	heard_log.flush();
// remove the colon and checksum
	rx_text.erase(0, p+3);
...
// double-nibble characters
		...
		if (curr_ch > 0) {
			...
			if (b_bot) {
				s2n = 3;
				for (int i = 0; i < SQLFILT_SIZE; i++) snfilt->run(s2n);
				ch_sqlch_open = true;
			}
			...
			if (fsq_squelch_open()) {
				write_rx_ch(curr_ch);
				if (b_bot)
					display_fsq_mon_text( "<bot>", FTextBase::CTRL);
				if (b_eol)
					display_fsq_mon_text( "<eol>", FTextBase::CTRL);
				if (b_eot)
					display_fsq_mon_text( "<eot>", FTextBase::CTRL);
			}

			if ( valid_char(curr_ch) || curr_ch == '\b' ) {
...
void fsq::transmit(double *buf, int len)
{
	if (xmtfilt && progdefaults.fsq_xmtfilter)
...
void fsq_transmit(void *who)
{
	fsq *me = (fsq *)who;
	try_tries = 16;
	Fl::add_timeout(me->xmtdelay(), try_transmit, me);
}
