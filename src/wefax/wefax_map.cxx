// ----------------------------------------------------------------------------
// wefax_map.cxx rgb wefax_map viewer
//
...
void wefax_map::resize_zoom(int x, int y, int w, int h)
{
	if( zoom < 0 ) {
		int stride = -zoom + 1 ;
...
void wefax_map::resize_height(int new_height, bool clear_img)
{
...
void wefax_map::stretch(double the_ratio)
{
...
void wefax_map::set_zoom( int the_zoom )
{
	zoom = the_zoom ;
...
void wefax_map::draw_cb( void *data, int x_screen, int y_screen, int wid_screen, uchar * __restrict__ buf) 
{
	const wefax_map * __restrict__ ptr_pic = ( const wefax_map * ) data ;
	...
	if(ptr_pic->binary) {
		...
		abort(); // This should never be called, see optimization in wefax_map::draw().
	}
} // wefax_map::draw_cb
...
void wefax_map::slant_corr(int x, int y)
{
	int row, col;
	unsigned char temp[width * depth];
...
int wefax_map::handle(int event)
{
...
bool wefax_map::restore( int row, int margin )
{
...
void wefax_map::erosion( int row )
{
...
void wefax_map::remove_noise( int row, int half_len, int noise_margin )
{
...
int wefax_picbox::handle(int event)
{
	if (!Fl::event_inside(this))
		return 0;
