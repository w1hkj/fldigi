...
// CAP_IMG is set in cap iff image transfer supported
	...
	case MODE_MFSK32L:
		...
		basetone = 16;
		numtones = 16;
		_puncturing = false; // No puncturing: Use more robust FEC instead
		break;
	case MODE_MFSK64L:
		samplerate = 8000;
		symlen =  128;
...
// only modes with odd number of symbits need a vote
	...
	if(met < 255 / 2) fec_confidence -=  1 + fec_confidence / 4;
...
// shift to range 0...255
	...
	//  when symbits == 6, low and high bit are punctured
	if (6 == symbits && _puncturing) decodesymbol(128);
}
