...
{ int len=strlen(Msg)+1;
  if(dspRedspAllocArray(&TxMsg,len)) return -1;
  dspCopyArray(TxMsg,Msg,len); TxPtr=0; Code=0L;
  return 0; }
...
{ free(TxVect);     TxVect=NULL;
  free(dspPhaseCorr);  dspPhaseCorr=NULL;
...
*/
  ...
  for(i=0; i<DataCarriers; i++)
  { if(Encoder.Output[i]) // data bit = 1 => only dspPhase correction
    { TxVect[i]=(TxVect[i]+dspPhaseCorr[i])&mask; }
    else	     // data bit = 0 => dspPhase flip + dspPhase correction
...
{ int i,mask,left,right;
  ...
  { if(rand()&0x100) // faster & simpler random generator ?
    { TxVect[i]=(TxVect[i]+dspPhaseCorr[i]+left)&mask; } // turn left 90 degrees
    else
    { TxVect[i]=(TxVect[i]+dspPhaseCorr[i]+right)&mask; } // turn right 90 degrees
  }
...
{ int i,p;
  ...
  { if(dspRedspAllocArray(&IntlvPipe,IntlvSize)) goto Error;
    if(PreFill) for(i=0; i<IntlvSize; i++) IntlvPipe[i]=rand()&1;
	   else dspClearArray(IntlvPipe,IntlvSize);
...
{
  if(!dspPowerOf2(FFTlen)) goto Error;
  FFTmask=FFTlen-1;
  ...
  if(dspRedspAllocArray(&PwrIntegMid,ScanSize)) goto Error;
  dspClearArray(PwrIntegMid,ScanSize);
  if(dspRedspAllocArray(&PwrIntegOut,ScanSize)) goto Error;
  dspClearArray(PwrIntegOut,ScanSize);
  IntegPtr=0;
  NodspRMSize=StepsPerSymb*2*DataCarrSepar;
  if(dspRedspAllocArray(&NormPwr,NodspRMSize)) goto Error;
...
{ int i,c,p,n; double Sum;
  ...
  if(IntegPtr==0)
  { printf("NormPwr:\n");
    for(i=0; i<NodspRMSize; i+=2*DataCarrSepar)
...
{ int i,p;
  ...
  dspClearArray(DecodePipe,DecodeSize); DecodePtr=0;
  ...
  if(dspRedspAllocArray(&IntlvPipe,IntlvSize)) goto Error;
  dspClearArray(IntlvPipe,IntlvSize); IntlvPtr=0;
...
{ int s,i,k; double Min,Max,Sig,Noise,SNR; int MinPos,MaxPos,code;
...
{ int s;
  ...
  dspPowerMid=NULL; dspPowerOut=NULL;
...
{ int s;
  ...
  free(dspPowerMid); free(dspPowerOut);
...
{ int s;
  ...
  free(dspPowerMid); dspPowerMid=NULL;
  free(dspPowerOut); dspPowerOut=NULL;
  ...
  free(DatadspPhase); DatadspPhase=NULL;
  free(DatadspPhase2); DatadspPhase2=NULL;

  Decoder.Free();

  free(SpectradspPower); SpectradspPower=NULL;
}
...
*/
  ...
  if(SpectraDisplay)
  { if(dspRedspAllocArray(&SpectradspPower,WindowLen)) goto Error; }
...
//  ProcLine.Process(&TestOfs.Output);
  ...
  { SyncProcess(ProcLine.InpPtr+SyncProcPtr);
    ...
    if(SyncPtr==SymbPtr)
    { s1=SyncProcPtr-ProcdspDelay+((int)SyncSymbShift-SymbPtr*SyncStep);
      s2=s1+SymbolSepar/2;
...
{ int i,j,k,r,s,s2;
  double pI,pQ; dspCmpx Correl; dspCmpx *PrevSlice;
  double I,Q; double dI,dQ; double P,A;
  double w0,w1; double Fl,F0,Fu;
  dspCmpx SymbTime;
  double SymbConf,SymbShift,FreqOfs;
  double dspRMS; int Loops,Incl;
  ...
  { k=(ScanFirst+i)&WindowLenMask;
    ...
    dspLowPass2(&Correl,CorrelMid[SyncPtr]+i,CorrelOut[SyncPtr]+i,W1,W2,W5);
...
*/
    ...
    for(i=0; i<FitLen; i++) // symbol-shift dspPhase fitting
    { SymbFit[i].re=dspAmpl(CorrelAver[0][i])-dspAmpl(CorrelAver[2][i]);
      SymbFit[i].im=dspAmpl(CorrelAver[1][i])-dspAmpl(CorrelAver[3][i]); }

//    P=dspFindMaxdspPower(SymbFit+30,4,j); j+=30;
    P=dspFindMaxdspPower(SymbFit+2,FitLen-4,j); j+=2;
...
//    printf(" => %2d",j);
    if(P>0.0)
    { SymbConf=dspAmpl(SymbFit[j]) + 0.5*(dspAmpl(SymbFit[j+1])+dspAmpl(SymbFit[j-1]));
      SymbConf*=0.5;
...
//      printf(" => (%5.2f,%5.2f,%5.2f) => %5.2f",Fl,F0,Fu,FreqOfs);
    ...
    if(SyncLocked)
    { // flip the SymbTime if it doesn't agree with the dspAverage
...
/* SYNC TEST */
    ...
    Loops=dspSelFitAver(SymbPipe,TrackPipeLen,(double)3.0,4,AverSymb,dspRMS,Incl);
    // printf(" AverSymb=[%+5.2f,%+5.2f], dspRMS=%5.3f/%2d",
    //	     AverSymb.re,AverSymb.im,dspRMS,Incl);
    // find dspAverage freq. offset
    Loops=dspSelFitAver(FreqPipe,TrackPipeLen,(double)2.5,4,AverFreq,dspRMS,Incl);
    SyncFreqDev=dspRMS;
...
//	    i, dspPhase.re,dspPhase.im, EvenSlice[i].re,EvenSlice[i].im);
    ...
    CdspcmpxMultAxB(Dtmp,Phas,Freq); Phas=Dtmp;
  }
...
//	   c,i,FFTbuff[c].re,FFTbuff[c].im,DataPwrOut[i]);
  dspLowPass2(dspPower(FFTbuff[c]),DataPwrMid[i],DataPwrOut[i],dW1,dW2,dW5);
...
//	   c,i,FFTbuff2[c].re,FFTbuff2[c].im,DataPwrOut[i]);
  dspLowPass2(dspPower(FFTbuff2[c]),DataPwrMid[i],DataPwrOut[i],dW1,dW2,dW5);
...
double MT63rx::SYNC_FreqOffset(void) { return SyncFreqOfs/DataCarrSepar; }

double MT63rx::SYNC_FreqDevdspRMS(void) { return SyncFreqDev/DataCarrSepar; }

double MT63rx::SYNC_TimeOffset(void) { return SyncSymbShift/SymbolSepar; }

double MT63rx::FEC_SNR(void) { return Decoder.SignalToNoise; }
