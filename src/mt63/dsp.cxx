...
{ double Max,Pwr; int i;
  Max=dspPower(Data[0]);
  for(i=1; i<Len; i++)
  { Pwr=dspPower(Data[i]); if(Pwr>Max) Max=Pwr;
  } return Max; }

double dspFindMaxdspPower(dspCmpx *Data, int Len, int &MaxPos)
{ double Max,Pwr; int i,pos;
  Max=dspPower(Data[0]); pos=0;
  for(i=1; i<Len; i++)
  { Pwr=dspPower(Data[i]); if(Pwr>Max) { Max=Pwr; pos=i; }
...
double WindowHamming(double dspPhase) { return cos(dspPhase/2); } // not exactly ...

double dspWindowHanning(double dspPhase) { return (1.0+cos(dspPhase))/2; }
...
{ return 0.35875+0.48829*cos(dspPhase)
	+0.14128*cos(2*dspPhase)+0.01168*cos(3*dspPhase); }
...
{ for(; Len; Len--) (*dble++)=(*dspS16++)*Gain; }
int dspConvS16todouble(short int *dspS16, double_buff *dble, int Len, double Gain)
{ int err=dble->EnsureSpace(Len); if(err) return -1;
  dspConvS16todouble(dspS16,dble->Data,Len,Gain); dble->Len=Len; return 0; }
...
{ for(; Len; Len--) (*Out++)=sqrt(dspPower(*Inp++)); }
...
dspPulseLimiter::dspPulseLimiter() { Tap=NULL; }

dspPulseLimiter::~dspPulseLimiter() { free(Tap); }

void dspPulseLimiter::Free(void) { free(Tap); Tap=NULL; }
...
{ Len=TapLen; Thres=LimitThres*LimitThres;
  ...
  dspClearArray(Tap,Len); Ptr=0; PwrSum=0.0; return 0; }
...
{ dspLowPass2Coeff(Integ,W1,W2,W5);
  MaxSqr=Range*Range;
  PwrMid=0.0; PwrOut=0.0; dspRMS=0.0;
...
{ int i,Out; double Sqr,Sum;
  ...
  if(PwrOut<=0.0) dspRMS=0.0; else dspRMS=sqrt(PwrOut);
  return 0; }
...
{ MinMS=0.01; MaxMS=0.05;
  ...
  Level=75; Hold=(-IntegLen); AvedspRMS=0.0;
}
...
dspPeriodLowPass2::dspPeriodLowPass2() { TapMid=NULL; TapOut=NULL; }

dspPeriodLowPass2::~dspPeriodLowPass2() { free(TapMid); free(TapOut); Output.Free(); }
...
dspBoxFilter::dspBoxFilter() { Tap=NULL; }

dspBoxFilter::~dspBoxFilter() { free(Tap); }

void dspBoxFilter::Free(void) { free(Tap); Tap=NULL; Output.Free(); }
...
dspCmpxBoxFilter::dspCmpxBoxFilter() { Tap=NULL; }

dspCmpxBoxFilter::~dspCmpxBoxFilter() { free(Tap); }

void dspCmpxBoxFilter::Free(void) { free(Tap); Tap=NULL; Output.Free(); }
...
dspFirFilter::dspFirFilter() { Tap=NULL; ExternShape=1; }
dspFirFilter::~dspFirFilter() { free(Tap); if(!ExternShape) free(Shape); }
...
int dspFirFilter::ComputeShape(double LowOmega, double UppOmega,
			double (*Window)(double))
{ if(ExternShape) { Shape=NULL; ExternShape=0; }
  if(dspRedspAllocArray(&Shape,Len)) return -1;
  dspWinFirI(LowOmega,UppOmega,Shape,Len,Window); return 0; }
...
dspQuadrSplit::dspQuadrSplit() { ExternShape = 1; }
...
dspQuadrComb::dspQuadrComb() { Tap=NULL; ExternShape=1; }
...
{ int err,i,o,r,t,len;
  dspCmpx *Inp; double *Out; int InpLen; double I,Q;
  InpLen=Input->Len;
...
dspCmpxMixer::dspCmpxMixer() { dspPhase=0.0; Omega=0.0; }

void dspCmpxMixer::Free(void) { Output.Free(); }
...
int dspCmpxMixer::ProcessFast(double *InpI, double *InpQ, int InpLen,
			   double *OutI, double *OutQ)
...
dspFMdemod::dspFMdemod() { PrevdspPhase=0.0; RefOmega=0.0; }

int dspFMdemod::Preset(double CenterOmega) { RefOmega=CenterOmega; return 0; }
...
dspRateConvLin::dspRateConvLin() { PrevSdspAmple=0; OutdspPhase=0; OutStep=1.0; }

void dspRateConvLin::SetOutVsInp(double OutVsInp) { OutStep=1.0/OutVsInp; }

void dspRateConvLin::SetInpVsOut(double InpVsOut) { OutStep=InpVsOut; }
...
{ int i; for(i=0; i<4; i++) Tap[i]=0;
  OutStep=1.0; OutdspPhase=0; TapPtr=0; }

void dspRateConvQuadr::SetOutVsInp(double OutVsInp) { OutStep=1.0/OutVsInp; }

void dspRateConvQuadr::SetInpVsOut(double InpVsOut) { OutStep=InpVsOut; }

int dspRateConvQuadr::Process(double *Inp, int InpLen,
...
dspRateConvBL::~dspRateConvBL() { Free(); }
...
void dspRateConvBL::SetOutVsInp(double OutVsInp) { OutStep=1.0/OutVsInp; }

void dspRateConvBL::SetInpVsOut(double InpVsOut) { OutStep=InpVsOut; }
...
{ int i,o,idx,t,err;
  double *Inp,*Out; int InpLen;
  double Sum0,Sum1; double *shape; double d;
  Inp=Input->Data; InpLen=Input->Len;
  err=Output.EnsureSpace((int)ceil(InpLen/OutStep)+2); if(err) return err;
  Out=Output.Data;
  if((Len+InpLen)>TapSize)
...
{ dspCmpx *Inp=Input->Data; int InpLen=Input->Len;
  int i,len,err;
  Output.Len=0;
  while(InpLen>0)
...
dspFFT_TimeShift::dspFFT_TimeShift() { FreqTable=NULL; }

dspFFT_TimeShift::~dspFFT_TimeShift() { free(FreqTable); }

void dspFFT_TimeShift::Free(void) { free(FreqTable); FreqTable=NULL; }

int dspFFT_TimeShift::Preset(int FFTlen, int Backwards)
{ int i; double p;
  dspPhase=0; Len=FFTlen; LenMask=FFTlen-1;
...
{ int i;
  ...
  DiffTapPtr=0; BitPtr=0; SyncdspPhase=0.0;
  SyncDrift=SyncDrift0=0; SyncConfid=0.0;
  dspLowPass2Coeff((double)IntegLen*2,W1,W2,W5);
}

dspDiffBitSync4::~dspDiffBitSync4() { free(InpTap); }
...
double dspDiffBitSync4::GetSyncConfid() { return 4*SyncConfid; }

double dspDiffBitSync4::GetSyncDriftRate() { return SyncDrift/4; }
...
// constructor
dsp_r2FFT::dsp_r2FFT() { BitRevIdx=NULL; Twiddle=NULL; /* Window=NULL; */ }

// destructor: free twiddles, bit-reverse lookup and window tables
dsp_r2FFT::~dsp_r2FFT() { free(BitRevIdx); free(Twiddle); /* free(Window); */ }
...
// bit reverse (in place) the dspSequence (before the actuall FTT)
...
{ int err,idx,ridx,mask,rmask; double dspPhase;
  if(!dspPowerOf2(size)) goto Error;
  Size=size;
  err=dspRedspAllocArray(&BitRevIdx,Size); if(err) goto Error;
...
{ int err,i;
  ...
  err=dspRedspAllocArray(&FFTbuff,Size); if(err) goto Error;

  err=dspRedspAllocArray(&SlideBuff,Size); if(err) goto Error;
...
{ int err,len,i,t; int InpLen; double *Inp;
...
{ int err,i;
  ...
  dspRedspAllocArray(&FFTbuff,Size); if(err) goto Error;

  for(i=0; i<2; i++)
  { err=dspRedspAllocArray(&Spectr[i],Size/2); if(err) goto Error; }

  err=dspRedspAllocArray(&SlideBuff,Size); if(err) goto Error;
  for(i=0; i<Size; i++) SlideBuff[i]=0.0;
  SlidePtr=0; Slide=0; Dist=step; Left=Dist;

  err=dspRedspAllocArray(&OvlapBuff,Size); if(err) goto Error;
...
{ int err,len,i,t; int InpLen; double *Inp,*Out;
  Inp=Input->Data; InpLen=Input->Len; Output.Len=0;
  while(InpLen)
  { for(i=len=dspIntmin(InpLen,Left); i; i--)
	{ SlideBuff[SlidePtr++]=(*Inp++); SlidePtr&=SizeMask; }
