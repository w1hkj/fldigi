...
#include <math.h>
#include "fl_disp_slider.h"

void fl_disp_slider::_fl_disp_slider() {
  value_ = 0;
  disp_value_ = 0;
  slider_ = 0; // FL_UP_BOX;
}

fl_disp_slider::fl_disp_slider(int X, int Y, int W, int H, const char* l)
: Fl_Valuator(X, Y, W, H, l) {
  box(FL_DOWN_BOX);
  _fl_disp_slider();
}

fl_disp_slider::fl_disp_slider(uchar t, int X, int Y, int W, int H, const char* l)
  : Fl_Valuator(X, Y, W, H, l) {
  type(t);
...
void fl_disp_slider::value(double v) {
  if (v <  0) v = 0;
  if (v > 1) v = 1;
  if (value_ != float(v)) {
    value_ = float(v); 
...
void fl_disp_slider::bounds(double a, double b) {
...
//int fl_disp_slider::scrollvalue(int p, int W, int t, int l) {
...
//  step(1, 1);
//  if (p+W > t+l) l = p+W-t;
//  value(W >= l ? 1.0 : double(W)/double(l));
//  bounds(t, l-W+t);
//  return value(p);
//}
...
void fl_disp_slider::draw_bg(int X, int Y, int W, int H) {
  fl_push_clip(X, Y, W, H);
  draw_box();
//  draw_box(FL_FLAT_BOX, X, Y, W, H, FL_YELLOW);
  fl_pop_clip();

  Fl_Color black = active_r() ? FL_FOREGROUND_COLOR : FL_INACTIVE_COLOR;
  if (type() == FL_VERT_NICE_SLIDER || type() == FL_VERT_FILL_SLIDER ||
	  type() == FL_VERT_DISP_SLIDER ) {
    draw_box(FL_THIN_DOWN_BOX, X+W/2-2, Y, 4, H, black);
  } else if (type() == FL_HOR_NICE_SLIDER || type() == FL_HOR_NICE_SLIDER ||
			 type() == FL_HOR_DISP_SLIDER) {
    draw_box(FL_THIN_DOWN_BOX, X, Y+H/2-2, W, 4, black);
  }
}

void fl_disp_slider::draw_(int X, int Y, int W, int H) {
  ...
  if (type()==FL_HOR_FILL_SLIDER || type() == FL_VERT_FILL_SLIDER) {
    ...
    if (type() == FL_VERT_NICE_SLIDER || type() == FL_VERT_DISP_SLIDER ||
		type() == FL_HOR_NICE_SLIDER  || type() == FL_HOR_DISP_SLIDER ) T += 4;
    if (S < T) S = T;
    xx = int(val*(ww-S)+.5);
  }
  int xsl, ysl, wsl, hsl;
  ...
  else if (type() == FL_HOR_NICE_SLIDER || type() == FL_HOR_DISP_SLIDER) {
    draw_box(box1, xsl, ysl, wsl, hsl, FL_GRAY);
    int d = (wsl-4)/2;
...
void fl_disp_slider::draw() {
  if (damage()&FL_DAMAGE_ALL) draw_box();
  draw_(x()+Fl::box_dx(box()),
       y()+Fl::box_dy(box()),
       w()-Fl::box_dw(box()),
       h()-Fl::box_dh(box()));
}

int fl_disp_slider::handle_(int event, int X, int Y, int W, int H) {
  switch (event) {
  case FL_PUSH:
  ...
  case FL_DRAG: {
	...
	if (offcenter < -10 || offcenter > 10) offcenter = 0;
      ...
      S = int(value_*ww+.5); if (S >= ww) return 0;
      int T = (horizontal() ? H : W)/2+1;
      if (type() == FL_VERT_NICE_SLIDER || type() == FL_VERT_DISP_SLIDER ||
		  type() == FL_HOR_NICE_SLIDER  || type() == FL_HOR_DISP_SLIDER) T += 4;
      if (S < T) S = T;
...
int fl_disp_slider::handle(int event) {
