//
// "$Id: Fl_Text_Display.cxx 7462 2010-04-06 23:00:56Z matt $"
//
...
void Fl_Text_Display_mod::overstrike(const char* text) {
  ...
  indent = startIndent;
  for ( c = text; *c != '\0'; c++ )
    indent += Fl_Text_Buffer_mod::character_width( c, indent, buf->tab_distance() );
  endIndent = indent;
  ...
  for ( p = startPos; ; p++ ) {
    ...
    if ( ch == '\n' )
      break;
    indent += Fl_Text_Buffer_mod::character_width( &ch, indent, buf->tab_distance() ); // FIXME: not unicode
    if ( indent == endIndent ) {
      p++;
      break;
...
**
   ...
   visLineLength.
*/
int Fl_Text_Display_mod::line_end(int pos, bool startPosIsLineStart) const {
    int retLines, retPos, retLineStart, retLineEnd;
...
*/
int Fl_Text_Display_mod::position_to_line( size_t pos, int *lineNum ) const {
  int i;
...
*/
int Fl_Text_Display_mod::string_width( const char *string, int length, int style ) const {
  Fl_Font font;
  int fsize;
    ...
    for (p=lineStart; p<buf->length(); p++) {
      // FIXME: character is ucs-4
    	...
    	if (c == '\n') {
          ...
          colNum += Fl_Text_Buffer_mod::character_width((char*)&c, colNum, tabDist); // FIXME: unicode
    	    if (countPixels)
...
*/
int Fl_Text_Display_mod::measure_proportional_character(char c, int colNum, int pos) const {
    int charLen, style;
    char expChar[ FL_TEXT_MAX_EXP_CHAR_LEN ];
    Fl_Text_Buffer_mod *styleBuf = mStyleBuffer;
    
  charLen = Fl_Text_Buffer_mod::expand_character(&c, colNum, expChar, buffer()->tab_distance()); // FIXME: unicode
    if (styleBuf == 0) {
	style = 0;
    } else {
      // FIXME: character is ucs-4
...
*/
int Fl_Text_Display_mod::range_touches_selection(const Fl_Text_Selection *sel,
...
//
// End of "$Id: Fl_Text_Display.cxx 7462 2010-04-06 23:00:56Z matt $".
//
